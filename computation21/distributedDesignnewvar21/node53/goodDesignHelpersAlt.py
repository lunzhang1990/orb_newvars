...
def isInRegion(p,params):
    p1 ,p2 = p
    theta1, theta2, mu, gamma, kappa, eta, e, f, = params
    
    first = (2/(mu*(e-f)) <= (p1+p2) <= (kappa/eta + gamma)*(eta+1)/(kappa+1)*2/mu*1/(e-f))
    second = (abs(p1-p2) <=(kappa/eta+gamma)*(1+eta)/(1+kappa)*2/mu*1/(e+f))
...
def isAttractorOverall(p, parameters,H1,H2):
    ...
    def computeFixedPoint(p):
        p1,p2 = p
        a, b = (kappa+1)/(kappa+H1(p1,theta1)), (kappa+1)/(kappa+H1(p2,theta1))
        m1 = (mu+2*b)/(mu*(mu+a+b))
        m2 = 2/mu - m1
    ...
    def P1(fp):
        ...
        m1,m2,p1,p2 = fp
        c,d = (kappa/eta + gamma*H2(p1,theta2))*(eta+H1(p1,theta1))/(kappa+H1(p1,theta1)), (kappa/eta+gamma*H2(p2,theta2))*(eta+H1(p2,theta1))/(kappa+H1(p2,theta1))
